...
DayGrid.mixin({
	...
	fgSegHtml: function(seg, disableResizing) {
		...
		if ( event.approved == 0 ){
			eventApproval = "<span class='event-needs-approval'></span>";
		}
		...
		if (seg.isStart) {
			...
			if (timeText) {
				timeHtml = '<span class="fc-time">' + eventApproval + htmlEscape(timeText) + '</span>';
			}
		}
	...
	fgSegHtml: function(seg, disableResizing) {
		...
		return '<a class="' + classes.join(' ') + '"' +
			...
			'>' +
				'<div class="fc-content">' +
					(timeText ?
						'<span class="event-needs-approval"></span><div class="fc-time"' +
